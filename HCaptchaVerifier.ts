...
import { AxiosWebClient, OnAxiosHttpError } from "@layer92/axios-utilities";

export class HCaptchaVerifier {
    private readonly _axios = new AxiosWebClient({});

    constructor(
        private readonly _secret: HCaptchaSecretBox
    ) {
     ...
     * @returns 
     */
    async verify_isSuccessfulAsync({tokenBox,onHttpError}:{tokenBox: HCaptchaResponseTokenBox, onHttpError?: OnAxiosHttpError}) {
